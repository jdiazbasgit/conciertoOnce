...
public class Bola extends Thread {
		...
		while (getVentanaArkanoid().getCuadrados().size() > 0) {
			...
			try {
				...
				getVentanaArkanoid().getCuadrados().forEach(c -> {
					if (miraChoque(c)) {
						RoundRectangle2D.Double miRectangulo = new RoundRectangle2D.Double(this.getPosicionX(),
								this.getPosicionY(), this.getDimension(), this.getDimension(), 5, 5);
	...
	private synchronized boolean miraChoque(Bloque bloque) {
		RoundRectangle2D.Double miRectangulo = new RoundRectangle2D.Double(this.getPosicionX(), this.getPosicionY(),
				this.getDimension(), this.getDimension(), 5, 5);
